...
import { parseISO, startOfMonth, endOfMonth, subMonths, isValid } from 'date-fns';
...
/**
 * Obtiene el valor de un activo para un mes específico
 */
const getAssetValueForMonth = (asset: Asset, monthEnd: Date): number => {
  ...
  const targetEnd = monthEnd.getTime();
  ...
  // Preferir uno dentro del mismo mes/año
  const sameMonth = candidates.find(c => 
    c._date.getMonth() === monthEnd.getMonth() && 
    c._date.getFullYear() === monthEnd.getFullYear()
  );
